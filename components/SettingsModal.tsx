...
import { X, Save, Database, Server, CheckCircle, Copy, ExternalLink, ClipboardCheck, Sparkles, Key, ListPlus, Trash2, Plus, Download, Upload } from 'lucide-react';
import { CustomFieldDefinition } from '../types';
import { useJobStore } from '../store/JobContext';
import { useToast } from '../store/ToastContext';
import { BACKEND_CODE_TEMPLATE } from '../constants';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleCopyCode = async () => {
    try {
      await navigator.clipboard.writeText(BACKEND_CODE_TEMPLATE);
      setCopied(true);
                      ...
                      <div className="space-y-2 w-full">
                        ...
                        <div className="relative group">
                            <pre className="bg-slate-900 text-slate-300 p-3 rounded-lg text-xs overflow-x-auto h-64 font-mono custom-scrollbar border border-slate-700">
                                <code>{BACKEND_CODE_TEMPLATE}</code>
                            </pre>
                        </div>
                      </div>
                    </div>
